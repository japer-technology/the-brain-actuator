...
This is the at-a-glance summary of every decision in the package, with options and
a recommended default for each. **Answer them in the one standard spot:**
[QUESTIONS.md](QUESTIONS.md). When every answer there is filled in, the proposal is
finalized.
...
| **D2c** | Concurrency baseline = one writer, many readers | yes / no | **Yes** |
| **D3** | Launch set of interfaces | see [03](03-interfaces.md) | **SDK + CLI + MCP** launch; REST + Console soon; rest later |
| **D4a** | Monorepo `core/` + `surfaces/*`, strict one-way deps | yes / no | **Yes** |
| **D4b** | Build/test posture (core vs. fixtures; surfaces as translators) | yes / no | **Yes** |
